...
- Updated the generated response-model provenance description and the pinned
  OpenAPI fixture's EOD-exposure example and schema description to match the
  deployed `eod_options_snapshot` label.
...
- `OASClient.exposure_eod(symbol, *, date=None)` for the new
  ...
  from platform-synced EOD options data (no chain upload required). Pass
